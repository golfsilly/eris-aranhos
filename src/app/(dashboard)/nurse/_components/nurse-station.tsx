    ...
    <main className="min-h-screen bg-gray-100 p-6 font-sans text-gray-800">
      <header className="mb-6 flex justify-between items-center bg-white p-4 rounded-xl shadow-sm">
        <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
          🏥 ระบบจัดการคิวห้องฉุกเฉิน (ER Nurse Station)
        </h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* คอลัมน์ซ้าย: ควบคุมหน้าจอวิกฤต */}
        <div className="space-y-6">
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200">
            <h2 className="text-lg font-bold mb-4 text-red-600 flex items-center gap-2">
              🚨 ควบคุมสถานะวิกฤต (TV Control)
            </h2>
            <button
              onClick={toggleCpr}
              className={`w-full py-8 rounded-xl text-2xl font-black transition-all transform active:scale-95 shadow-md ${
                isCprActive
            ...
            >
              {isCprActive
                ? "🔴 กำลังแสดงหน้าจอ CPR (กดเพื่อปิด)"
                : "⚪ เปิดหน้าจอ CPR บน TV"}
            </button>
          </div>
        </div>
