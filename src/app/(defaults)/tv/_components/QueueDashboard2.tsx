...
import { Clock, Activity, Users, AlertCircle } from "lucide-react";
...
export default function QueueDashboard2() {
  const [isCPR, setIsCPR] = useState(false);
  const [time, setTime] = useState("");
  const [videos, setVideos] = useState([]);
  const [currentIndex, setCurrentIndex] = useState(0);
  const activeVideos = videos.filter((x) => x.isActive);
                ...
                {isCPR ? (
                  <div className="absolute inset-0 flex items-center justify-center bg-red-950">
                    <div className="text-center px-6">
                      <div className="animate-pulse text-red-500 text-5xl font-black tracking-wide mb-4">
                        CPR
                      </div>
                      ...
                      <div className="mt-4 text-red-300 text-sm">
                        ขอความร่วมมือหลีกเลี่ยงบริเวณดังกล่าว
                      </div>
                    </div>
                  </div>
                ) : (
